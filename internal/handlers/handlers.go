...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't insert the reservation")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't insert room restirction")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
func (m *Repository) BookRoom(w http.ResponseWriter, r *http.Request) {
	...
	// mydata.Sd = startDate
	// mydata.Ed = endDate
	// data, _ := json.MarshalIndent(mydata, " ", " ")
	// w.Header().Set("Content-Type", "application/json")
	// w.Write(data)
}
...
func (m *Repository) AdminReservationShow(w http.ResponseWriter, r *http.Request) {
	expanded := strings.Split(r.RequestURI, "/")
	id, err := strconv.Atoi(expanded[4])
	if err != nil {
		helpers.ServerError(w, err)
	}
...
func (m *Repository) AdminPostUpdateReservation(w http.ResponseWriter, r *http.Request) {
	...
	reservation.Email = r.FormValue("email")
	reservation.FirstName = r.FormValue("first_name")
	reservation.LastName = r.FormValue("last_name")
	reservation.Phone = r.FormValue("phone")
	reservation.ID = id
